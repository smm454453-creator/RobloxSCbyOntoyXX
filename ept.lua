...
-- ── REMOTE CONFIG ─────────────────────────────────────────────────────────────
-- Ganti nama remote di sini sesuai hasil dari Remote Spy
local REMOTE_ATTACK   = "AttackPlayer"    -- RemoteEvent nama serangan
local REMOTE_SKILL    = "UseSkill"        -- RemoteEvent skill utama
-- Tambah remote lain di sini jika perlu
...
local function FindAttackRemote()
    -- Cari di ReplicatedStorage dulu, fallback ke seluruh game
...
local lastAttack = 0
local ATTACK_RATE = 0.1  -- fire tiap 100ms per target
...
local function TickAutoAttack()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        -- Fallback: cari remote dari tool yang sedang dipegang
        local char = GetChar()
        if char then
...
-- ── NO COOLDOWN — namecall hook ───────────────────────────────────────────────
-- Intercept semua FireServer yang namanya cocok dengan REMOTE_SKILL
-- dan skip cooldown check dengan fire langsung tanpa throttle
    ...
    old = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if CONFIG.NoCooldown then
            if (m == "FireServer" or m == "InvokeServer") then
                ...
                -- Fire skill remote langsung tanpa delay
                ...
                or name == REMOTE_SKILL then
                    -- Fire dua kali — satu normal, bypass cooldown window
...
-- ── FAST RUN — GetPropertyChangedSignal reset-proof ──────────────────────────
local wsConn = nil
...
sg.Name = "Ontoy_EPT"; sg.ResetOnSpawn = false
...
titleLbl.Size = UDim2.new(1,-140,1,0); titleLbl.Position = UDim2.new(0,30,0,0)
...
byLbl.Size = UDim2.new(0,60,1,0); byLbl.Position = UDim2.new(0,290,0,0)
...
local function Toggle(parent, label, sub, ac)
    ...
    local function Set(s)
        ...
        if s then
            ...
            row.BackgroundColor3 = Color3.fromRGB(14,10,28); stroke.Color = C.AccentDim
        else
...
local function Slider(parent, label, sub, dMin, dMax, initVal, unit, onChange)
    ...
    local row = Instance.new("Frame", parent)
    row.Size = UDim2.new(1,-8,0,72); row.BackgroundColor3 = C.BG2; row.BorderSizePixel = 0
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,8)
    ...
    if sub then
        local s = Instance.new("TextLabel", row)
        s.Size = UDim2.new(1,-80,0,14); s.Position = UDim2.new(0,14,0,26)
        s.BackgroundTransparency = 1; s.Text = sub; s.TextColor3 = C.TextSub
        s.Font = Enum.Font.Gotham; s.TextSize = 10; s.TextXAlignment = Enum.TextXAlignment.Left
    end
    ...
    sbg.Size = UDim2.new(1,-28,0,5); sbg.Position = UDim2.new(0,14,0,50)
...
local _, aaGet, _ = Toggle(pgCombat, "Auto Attack",
    "Fire RemoteEvent serangan ke player dalam range", C.Orange)
...
local _, ncGet, _ = Toggle(pgCombat, "No Cooldown",
    "Namecall hook — bypass cooldown skill/ability", C.Accent)
...
local _, ncClipGet, _ = Toggle(pgMisc, "Noclip",
    "RunService.Stepped — CanCollide false tiap frame", C.Green)
