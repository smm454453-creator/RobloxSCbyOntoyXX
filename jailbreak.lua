...
-- ── INFINITE AMMO ────────────────────────────────────────────────────────────
-- Jailbreak ammo disimpan di IntValue/NumberValue di dalam tool
-- DAN di StringValue bernama "Ammo" atau attribute — scan semua descendant
local function ApplyInfAmmo()
	local c = LP.Character; if not c then return end
	local tool = c:FindFirstChildOfClass("Tool"); if not tool then return end
	-- scan NumberValue dan IntValue
	for _, v in ipairs(tool:GetDescendants()) do
		if v:IsA("NumberValue") or v:IsA("IntValue") then
			local n = string.lower(v.Name)
			if n:find("ammo") or n:find("clip") or n:find("mag") or n:find("bullet") or n:find("round") then
				if v.Value < 999 then pcall(function() v.Value = 999 end) end
			end
		end
	end
	-- scan attribute (beberapa game pakai ini)
	pcall(function()
		local attrs = tool:GetAttributes()
		for name, val in pairs(attrs) do
			local n = string.lower(name)
			if (n:find("ammo") or n:find("clip") or n:find("mag")) and type(val) == "number" then
				if val < 999 then tool:SetAttribute(name, 999) end
			end
		end
	end)
end
	...
	if CONFIG.NoRecoil or CONFIG.InfAmmo then
		...
		for _, v in ipairs(tool:GetDescendants()) do
			if v:IsA("NumberValue") or v:IsA("IntValue") then
				...
				if CONFIG.InfAmmo and (n:find("ammo") or n:find("clip") or n:find("mag") or n:find("bullet") or n:find("round")) then
					if v.Value < 999 then pcall(function() v.Value = 999 end) end
				end
			end
		end
		if CONFIG.InfAmmo then
			pcall(function()
				...
				for name, val in pairs(attrs) do
					local n = string.lower(name)
					if (n:find("ammo") or n:find("clip") or n:find("mag")) and type(val) == "number" and val < 999 then
...
-- ── ANTI RAGDOLL ─────────────────────────────────────────────────────────────
-- Jailbreak pakai ragdoll saat freefall + parasut ga dibuka = tiduran
-- Harus disable: Ragdoll, FallingDown, GettingUp, Physics state
-- DAN force UpDirection biar ga kebalik
...
local function ApplyAntiRagdoll()
	...
	pcall(function()
		...
		-- kalau lagi di state ragdoll, force ke Landed
		local cur = hum:GetState()
		if cur == Enum.HumanoidStateType.Ragdoll
		or cur == Enum.HumanoidStateType.FallingDown then
...
-- FIX: pastikan label selalu dibuat saat MakeHL dipanggil
...
local function RemoveHL(p)
	if ESP_HL[p]  then pcall(function() ESP_HL[p]:Destroy() end); ESP_HL[p] = nil end
	if ESP_LBL[p] then pcall(function() ESP_LBL[p]:Remove() end); ESP_LBL[p] = nil end
	if ESP_TR[p]  then pcall(function() ESP_TR[p]:Remove()  end); ESP_TR[p]  = nil end
end

-- FIX: UpdateESP pakai pcall per-player biar satu error ga break semua
local function UpdateESP()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		...
		local ok, err = pcall(function()
			...
			if not (pc and proot and phum and phum.Health > 0) then
				lbl.Visible = false
			...
			if on and myRoot then
				...
				lbl.Position = Vector2.new(sc.X, sc.Y - 28)
		...
		if not ok then
			pcall(function() lbl.Visible = false end)
		end
	end

	-- cleanup player yang udah leave
	for p in pairs(ESP_LBL) do
		if not p.Parent then RemoveHL(p) end
	end
end
...
local ws2 = Instance.new("UIStroke", win); ws2.Color = REDZ.Stroke; ws2.Thickness = 1.5
...
local _, infAmmoGet, _  = Toggle(pgCombat, "Infinite Ammo",
	"Set ammo/clip/mag ke 999 + attribute scan", REDZ.Green)
...
local _, antiRagdollGet, _ = Toggle(pgMove, "Anti-Ragdoll",
	"Disable Ragdoll + FallingDown + GettingUp state", REDZ.Green)
...
local _, espGet, _    = Toggle(pgESP, "ESP Highlight",
	"Highlight + label nama + jarak — Biru=Police Merah=Crim")
...
Wire(antiRagdollGet, "AntiRagdoll", nil, DisableAntiRagdoll)
...
Wire(tracerGet, "Tracers", nil, function()
	for p, ln in pairs(ESP_TR) do
		pcall(function() ln:Remove() end); ESP_TR[p] = nil
	end
end)
...
closeBtn.MouseButton1Click:Connect(function()
	...
	for p in pairs(ESP_HL) do RemoveHL(p) end
	for p, ln in pairs(ESP_TR) do pcall(function() ln:Remove() end); ESP_TR[p] = nil end
...
LP.CharacterAdded:Connect(function(char)
	...
	if CONFIG.AntiRagdoll then ApplyAntiRagdoll() end
...
RunService.RenderStepped:Connect(function()
	...
	elseif not CONFIG.ESP then
		for _, lbl in pairs(ESP_LBL) do pcall(function() lbl.Visible = false end) end
		for _, tr  in pairs(ESP_TR)  do pcall(function() tr.Visible  = false end) end
	end
end)
