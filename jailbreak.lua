...
-- ── SILENT AIM — namecall hook (sistem jailbreak) ────────────────────────────
local function GetBestTarget()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if d < CONFIG.SilentFOV and d < bestD then
            bestD = d; best = bone
        end
    end
    return best
...
-- ── INFINITE JUMP — JumpPower lock + repeat on JumpRequest ───────────────────
-- Sama seperti jailbreak HighJump tapi trigger on JumpRequest tiap frame
local jpConn   = nil
local ijConn   = nil
...
local function StartInfiniteJump()
    ...
    -- Infinite jump: tiap JumpRequest, set state jumping lagi
...
local function UpdateESP()
    ...
    for plr, d in pairs(espData) do
        if not plr or not plr.Parent then
            RemoveESP(d); espData[plr] = nil
        end
    end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP and not espData[plr] then
            espData[plr] = CreateESP()
        end
    end
...
local _, saGet, _ = Toggle(pgCombat, "Silent Aim",
    "Namecall hook: Fire/FireServer/Raycast", C.Red)
BoneCycle(pgCombat)
