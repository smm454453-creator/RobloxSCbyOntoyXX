...
local function GetFarmData()
    ...
    MyLevel = level
    pcall(RelzCheckQuest)
    if not Mon or not CFrameQuest or not CFrameMon then
        return nil, level
    end
...
local function StartNearestFarm()
    ...
    nearestFarmConn = RunService.Heartbeat:Connect(function()
        ...
        if not mob then
            farmStatus = "Nearest: waiting for mob..."
            return
        end
        local mr = mob:FindFirstChild("HumanoidRootPart")
        ...
        local hoverY = math.max(mr.Position.Y + CONFIG.FarmHoverHeight, MIN_FLY_Y)
        local hoverCF = CFrame.new(mr.Position.X, hoverY, mr.Position.Z)
        local dist = (hoverCF.Position - root.Position).Magnitude
        if dist > 8 then
            if not waitingFly then
                waitingFly = true
                FlyTo(hoverCF, function()
                    waitingFly = false
                end)
...
local _, farmGet, farmSet = MakeToggleRow(farmPage, "Auto Farm",
    "Relz route → NPC → quest → hover attack", REDZ.Green)
