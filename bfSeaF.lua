...
local function StopWaterWalk()
	if S.waterWalkConn then S.waterWalkConn:Disconnect(); S.waterWalkConn = nil end
	if S.waterPlatform then S.waterPlatform:Destroy(); S.waterPlatform = nil end
end
...
local function StopClearVision()
	if S.clearVisionConn then S.clearVisionConn:Disconnect(); S.clearVisionConn = nil end
end
...
local function StopHoverBoat()
	if S.hoverBoatConn then S.hoverBoatConn:Disconnect(); S.hoverBoatConn = nil end
end
...
local function StopBoatSpeed()
	if S.boatSpeedConn then S.boatSpeedConn:Disconnect(); S.boatSpeedConn = nil end
end
...
local function StopSmoothCollision()
	if S.collisionConn then S.collisionConn:Disconnect(); S.collisionConn = nil end
end

local function StartWalkSpeed()
	if S.walkSpeedConn then return end
	S.walkSpeedConn = RunService.Heartbeat:Connect(function(dt)
		local hum = GetHumanoid()
		local root = GetRoot()
		if hum and root and hum.MoveDirection.Magnitude > 0 then
			local extraSpeed = S.walkSpeedValue - 16
...
local function StartJumpPower()
	...
	S.jumpPowerConn = RunService.RenderStepped:Connect(function()
		local hum = GetHumanoid()
		if hum then
...
local function StopJumpPower()
	if S.jumpPowerConn then S.jumpPowerConn:Disconnect(); S.jumpPowerConn = nil end
end
...
local function MakeSliderRow(label, dMin, dMax, initPct, unit, onChanged)
	...
	hitbox.MouseButton1Click:Connect(function() Apply(Compute(UserInputService:GetMouseLocation().X)) end)
end

MakeSectionLabel("WATER SURFACE PLATFORM")
local waterWalkGet = MakeToggleRow("Water Walk", "Part transparan agar bisa jalan di atas air")
MakeSliderRow("Platform Y Offset", 0, 100, 0.2, " Y", function(val) S.waterWalkHeight = val end)

MakeSectionLabel("CHARACTER MOVEMENT")
local walkSpeedGet = MakeToggleRow("Fast Run", "Custom WalkSpeed untuk lari kencang")
MakeSliderRow("WalkSpeed", 16, 300, 0, "", function(val) S.walkSpeedValue = val end)
local jumpPowerGet = MakeToggleRow("High Jump", "Custom JumpPower untuk lompat tinggi")
MakeSliderRow("JumpPower", 50, 500, 0, "", function(val) S.jumpPowerValue = val end)

MakeSectionLabel("LIGHTING")
local clearVisionGet = MakeToggleRow("Clear Vision", "Hapus kabut dan jadikan terang benderang")

MakeSectionLabel("VEHICLE PHYSICS")
local hoverBoatGet = MakeToggleRow("Hover Boat", "Paksa kapal melayang di atas permukaan air")
MakeSliderRow("Hover Height", 0, 100, 0.45, " Y", function(val) S.hoverHeight = val end)
local boatSpeedGet = MakeToggleRow("Boat Speed", "Dorong kapal maju saat W ditekan")
MakeSliderRow("Speed Value", 10, 500, 0.18, " stud/s", function(val) S.boatSpeedValue = val end)

MakeSectionLabel("COLLISION MODIFIER")
local collisionGet = MakeToggleRow("Smooth Boat Collision", "Noclip Kapal Tembus Batu")
...
minimizeBtn.MouseButton1Click:Connect(function()
	...
	mainWindow.Size = contentVisible and UDim2.new(0,420,0,480) or UDim2.new(0,420,0,42)
end)

closeBtn.MouseButton1Click:Connect(function()
	StopWaterWalk(); StopWalkSpeed(); StopJumpPower(); StopClearVision(); StopHoverBoat(); StopBoatSpeed(); StopSmoothCollision()
	screenGui:Destroy()
end)
